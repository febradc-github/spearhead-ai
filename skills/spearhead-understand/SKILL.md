...
- All status mutations go through the state CLI: `node "$CLAUDE_PLUGIN_ROOT/scripts/state.js" <command> ... --dir <project root>` (if the variable is unset, use the plugin's install path). Never Write/Edit `spearhead-attacks/status.yml` -- guard.js blocks it.
...
1. If `spearhead-attacks/status.yml` does not exist (or the previous attack is `aborted`/`complete`): run `state.js init "<short title>"` (pass `--attack-counter <n>` from the archived attack when this follows an abort). This lazily creates `spearhead-attacks/`.
2. First run for this project: offer the repo-hygiene addition to `.gitignore`:
   ...
   and recommend committing the rest of `spearhead-attacks/` (it is the project's decision record). If the user declines, run `state.js set-gitignore-declined` and never ask again for this attack.
...
6. Write `spearhead-attacks/problem/PROBLEM.md`:
