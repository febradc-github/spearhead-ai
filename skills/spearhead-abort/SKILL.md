...
description: "Aborts the current attack with a recorded reason: artifacts archived to spearhead-attacks/archive/<timestamp>/, worktrees removed, branches left for salvage, counter bumped. History, not deletion. Dispatched by /spearhead:abort only."
...
<important>
- Abort is archival, not deletion: every phase artifact survives under `spearhead-attacks/archive/<timestamp>/`.
...
4. Remove any task worktrees: `git worktree remove --force spearhead-attacks/worktrees/T-<n>` for each present. Branches stay.
5. Archive: create `spearhead-attacks/archive/<ISO timestamp>/` and MOVE into it: `problem/`, `design/`, `plan/`, `verify/`, `ship/`, `retro/`, `decisions/` (those that exist), and `status.yml`. Never move `.remind-state.json` or `worktrees/`.
