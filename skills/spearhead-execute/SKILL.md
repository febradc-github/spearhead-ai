...
- Git preconditions: refuse (naming the requirement) if the project is not a git repository, or if `base_branch` has uncommitted changes outside `spearhead/`. The worktree/branch model depends on both.
...
- Branch `spearhead/T-<n>` is created from `base_branch`; worktree at `spearhead/worktrees/T-<n>/`:
  `git branch spearhead/T-<n> <base_branch> && git worktree add spearhead/worktrees/T-<n> spearhead/T-<n>`
- The coder works ONLY inside that worktree, committing small conventional commits on that branch. It never merges, never touches `base_branch`, never edits `spearhead/` state files, never uses `--no-verify`. Commit messages never tag Anthropic or Claude and never carry a "Co-Authored-By:" trailer (commit message rule in `rules/RULES.md`; guard.js enforces it).
...
1. Check gates: `state.js show`; `git rev-parse --is-inside-work-tree`; `git status --porcelain` on `base_branch` filtered to paths outside `spearhead/`. Refuse on any failure, naming the unmet requirement.
...
4. Create the branch and worktree (commands above). Then dispatch:
   `state.js transition T-<n> in_progress --branch spearhead/T-<n> --worktree spearhead/worktrees/T-<n> --mode <background|foreground>`
...
- `plan` not `approved`; not a git repo; dirty `base_branch` outside `spearhead/`: name the requirement.
