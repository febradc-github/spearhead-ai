...
//   ---
//   <body>
// `tags`/`related` may also be written as an explicit empty list (`[]`).
// Scalars are quoted (`"..."`) only when they contain characters that
// would otherwise be ambiguous (brackets, colons, hashes, surrounding
// whitespace); plain values are left bare, matching validate-state.js's
// `scalar()` convention.
//
...
const SCALAR_FIELDS = new Set(['type', 'source', 'updated']);
...
// 'unknown'); `tags`/`related`/`source`/`updated` are present only when the
// input supplied them. Never throws -- malformed or absent frontmatter
// falls back to `{ fields: { type: 'unknown' }, body: <original content> }`.
...
// defaults to 'unknown' when absent; `tags`/`related` are only emitted as
// block lists (or `[]`) when they are arrays; `source`/`updated` are only
// emitted when defined. For well-formed `fields`, `parseFrontmatter(
// serializeFrontmatter(fields, body))` round-trips to `{ fields, body }`.
function serializeFrontmatter(fields, body) {
  ...
  for (const key of ['source', 'updated']) {
