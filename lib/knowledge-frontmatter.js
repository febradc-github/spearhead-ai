...
// 'unknown'); `tags`/`related`/`source`/`updated`/`source_hash` are present
// only when the input supplied them. Never throws -- malformed or absent
// frontmatter falls back to `{ fields: { type: 'unknown' }, body: <original
// content> }`.
...
// block lists (or `[]`) when they are arrays; `source`/`updated`/
// `source_hash` are only emitted when defined. For well-formed `fields`,
// `parseFrontmatter(serializeFrontmatter(fields, body))` round-trips to
// `{ fields, body }`.
